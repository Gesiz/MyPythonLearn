...
class WebServer:
    def __init__(self):
        ...
        self.socket_client_addr = None
        self.socket_server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket_server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, True)
        self.socket_server.bind(("", 9999))
        self.socket_server.listen(128)

    def start_server(self):
        while True:
            self.socket_client, self.socket_client_addr = self.socket_server.accept()


    def recv_data(self):
        data = self.socket_client.recv(1024).decode()
        http_line = "HTTP/1.1 400 OK\r\n"
        http_header = "server:Python3.9\r\n"
        http_body = "\r\n"
